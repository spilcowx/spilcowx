...
## 🚀 Featured Project: SP.Engine
### **[.NET 8 기반 고성능 실시간 멀티플레이어 게임 서버 엔진](https://github.com/spilbum/SP.Engine)**
*대규모 동시 접속 환경의 고질적 한계인 GC 스파이크와 락 경합(Lock Contention)을 프레임워크 레벨에서 통제하기 위해 구축한 레퍼런스 아키텍처입니다.*
- **Concurrency (Lock-Free)**: Fiber 기반의 가상 스레드 큐와 객체 풀링을 내장하여, 데드락과 스레드 경합을 원천 차단한 안전한 비즈니스 런타임 구현
- **Zero-Allocation Pipeline**: 런타임 리플렉션 비용을 제거하기 위해 동적 코드 생성(IL Emit)을 활용한 초고속 패킷 직렬화 프레임워크 구축
- **Network Resilience**: 잦은 네트워크 전환 시 패킷 유실을 막는 무중단 세션 복구(Resumption) 로직 및 TCP/UDP 하이브리드 스택 설계
